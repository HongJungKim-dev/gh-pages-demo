const express = require('express');//DB brnach
...
app.post('/delete', async (req, res) => {//postDelete 함수 호출되면 안드로이에서 보낸 값 받아서 
  ...
  db.query(`DELETE FROM chat WHERE Db_user = ? AND Db_roomCode = ?`, [userName, roomCode], function (error, result3) {//안드로이드로 부터 받은 값의 user와 roomcode와 일치하는 users 테이블의 값을 삭제한다
    if (error) {
      throw error;
    }
  });
...
// http Server를 socket.io 서버로 업그레이드, 서버 시작
...
// 메세지 부분 
  socket.on('newMessage', (data) => {
    ...
    db.query(`INSERT INTO chat (Db_roomCode, Db_user,Db_chat) VALUES (?,?,?)`, [messageData.to, messageData.from, messageData.content], function (error, result5) {
        if (error) {
          throw error;
        }
      });
